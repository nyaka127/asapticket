...
import { calculateClientPrice, formatCurrency, getHistoricalPriceStatus } from '@/lib/pricing';
import { MAJOR_CITIES, getDistanceBetweenCodes } from '@/lib/geo';

function FlightsContent() {
  const searchParams = useSearchParams();
  const [origin, setOrigin] = useState(searchParams.get('origin') || 'NYC');
  const [destination, setDestination] = useState(searchParams.get('destination') || 'LHR');
  ...
  const searchFlights = async (e?: React.FormEvent) => {
    ...
    setSearched(true);
    try {
      const res = await fetch(`/api/flights/search?origin=${origin}&destination=${destination}&departureDate=${date}&returnDate=${tripType === 'roundtrip' ? returnDate : ''}&tripType=${tripType}&cabin=${cabin}`);
  ...
  useEffect(() => { if (searchParams.get('origin')) searchFlights(); }, []);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center">
      ...
      {showHeroChoice && (
        <div className="w-full bg-slate-900 py-16 px-4 border-b border-white/5 animate-in fade-in zoom-in-95 duration-700">
           <div className="max-w-4xl mx-auto text-center mb-12">
              <h1 className="text-5xl md:text-7xl font-black text-white italic tracking-tighter uppercase mb-6 leading-[0.9]">How do you want to <br/><span className="text-brand-secondary">secure your flights?</span></h1>
              <p className="text-white/40 font-bold max-w-xl mx-auto">Choose between a human expert to negotiate private fares or browse our live global inventory yourself.</p>
           </div>
      ...
      <div className={`w-full bg-brand-primary py-12 px-4 shadow-xl ${showHeroChoice ? 'opacity-20 blur-sm pointer-events-none' : 'animate-in fade-in duration-1000'}`}>
        <div className="max-w-7xl mx-auto">
          {/* Trip Type Selector */}
          <div className="flex gap-1 bg-white/10 p-1.5 rounded-2xl w-fit mb-6 overflow-hidden backdrop-blur-md border border-white/10">
          ...
          <form onSubmit={searchFlights} className="bg-white rounded-[2.5rem] p-8 space-y-8 shadow-2xl relative">
            <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-6 items-end">
              <div className="lg:col-span-1 relative group">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Origin</label>
                <input type="text" value={origin} onChange={e => setOrigin(e.target.value)}
                  placeholder="City/Airport" className="w-full border-none bg-slate-50 rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-brand-primary" />
                <div className="hidden group-focus-within:grid absolute top-full left-0 right-[-140px] z-[100] mt-4 bg-white rounded-[2rem] shadow-[0_0_80px_rgba(0,0,0,0.15)] p-8 grid-cols-1 gap-6 border border-slate-100 animate-in fade-in slide-in-from-top-4 duration-500 max-h-[500px] overflow-y-auto custom-scrollbar">
                  {[
                    { label: 'Americas Hubs', codes: ['JFK', 'LAX', 'ORD', 'YYZ', 'MIA'] },
                    { label: 'Europe Elite', codes: ['LHR', 'CDG', 'FRA', 'AMS', 'FCO', 'MAD'] },
                    { label: 'Africa Excellence', codes: ['NBO', 'EBB', 'ADD', 'KGL', 'LOS', 'JNB', 'ACC', 'DKR'] },
                    { label: 'Middle East & Gulf', codes: ['DXB', 'DOH', 'IST', 'RUH', 'AMM', 'CAI'] },
                    { label: 'Asia Excellence', codes: ['SIN', 'HND', 'BKK', 'ICN', 'HKG', 'PVG', 'MNL', 'DEL'] },
                    { label: 'Oceania & LatAm', codes: ['SYD', 'MEL', 'AKL', 'GRU', 'MEX', 'BOG', 'SCL'] }
                  ].map(cat => (
                    <div key={cat.label}>
              ...
              <div className="lg:col-span-1 relative group">
                ...
                <input type="text" value={destination} onChange={e => setDestination(e.target.value)}
                  placeholder="City/Airport" className="w-full border-none bg-slate-50 rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-brand-primary" />
                <div className="hidden group-focus-within:grid absolute top-full left-[-140px] right-0 z-[100] mt-4 bg-white rounded-[2rem] shadow-[0_0_80px_rgba(0,0,0,0.15)] p-8 grid-cols-1 gap-6 border border-slate-100 animate-in fade-in slide-in-from-top-4 duration-500 max-h-[500px] overflow-y-auto custom-scrollbar">
                  {[
                    { label: 'World Class Capitals', codes: ['HND', 'SYD', 'BKK', 'CDG', 'LHR', 'SIN'] },
                    { label: 'African Portals', codes: ['NBO', 'KGL', 'ADD', 'ACC', 'LOS', 'CPT'] },
                    { label: 'Middle East Hubs', codes: ['DXB', 'DOH', 'IST', 'RUH'] },
                    { label: 'Oceania & Americas', codes: ['AKL', 'SYD', 'MEL', 'GRU', 'MEX', 'BOG'] }
                  ].map(cat => (
                    <div key={cat.label}>
                       <div className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-3 border-b border-slate-50 pb-2">{cat.label}</div>
                       <div className="grid grid-cols-4 gap-2">
            ...
            <div className="flex gap-8 items-center border-t border-slate-100 pt-6">
               <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-3 bg-slate-50 px-3 py-1.5 rounded-xl">
                     <button type="button" onClick={() => setPassengers(Math.max(1, passengers - 1))} className="text-brand-primary font-black">-</button>
                     <span className="text-sm font-black text-slate-800">{passengers}</span>
                     <button type="button" onClick={() => setPassengers(passengers + 1)} className="text-brand-primary font-black">+</button>
                  </div>
               </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-white rounded-[2rem] p-4 shadow-xl border border-slate-200 overflow-x-auto scrollbar-none">
             <div className="flex gap-4 min-w-max">
                {[-3, -2, -1, 0, 1, 2, 3].map(offset => {
                   ...
                   if (s.isPeak) colorClass = "bg-orange-50 text-orange-700 border-orange-100 hover:bg-orange-100";
                   else if (s.isLow) colorClass = "bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-100";
                   else colorClass = "bg-amber-50 text-amber-700 border-amber-100 hover:bg-amber-100";
                   ...
                   return (
                      <button key={offset} onClick={() => { setDate(dStr); searchFlights(); }}
                        className={`px-6 py-3 rounded-2xl border-2 transition-all flex flex-col items-center gap-1 group ${colorClass}`}>
                         <span className={`text-[8px] font-black uppercase tracking-widest ${isSelected ? 'text-white/60' : 'text-slate-400'}`}>
                         ...
                         <span className={`text-[10px] font-black mt-1 ${isSelected ? 'text-white' : ''}`}>
                            {s.isPeak ? 'Peak' : s.isLow ? 'Best' : 'Avg'}
                         </span>
                      </button>
                   );
                })}
          ...
          <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-slate-400 px-2 mt-4">
             ...
             <div className="flex gap-4">
                <span className="text-emerald-500">🛡️ No-Loss Guard Active</span>
          ...
          {!loading && filteredFlights.length > 0 && (
             <div className="space-y-6">
                {filteredFlights.map((flight) => {
                  ...
                  const totalFormatted = formatCurrency(clientPricePerPerson * passengers);
                  ...
                  const arrCode = flight.arrival?.iataCode || destination;
                                ...
                                <div className="flex-1">
                                   ...
                                   <div className="flex gap-2 mb-6 -mt-4">
                                      <div className={`flex items-center gap-2 bg-${historical.color}-50 text-${historical.color}-600 px-4 py-1.5 rounded-full border border-${historical.color}-100 animate-in fade-in duration-500`}>
                                         ...
                                         {historical.diff > 0 && <span className="text-[8px] font-bold opacity-60">(${historical.diff.toFixed(0)} {historical.label.includes('Great') ? 'Savings' : 'Premium'})</span>}
                                      </div>
                                      {isCheapest && <span className="bg-emerald-500 text-white text-[8px] font-black px-3 py-1 rounded-lg uppercase tracking-widest shadow-lg shadow-emerald-500/20">Lowest Fare</span>}
                       ...
                       <div className="md:w-64 flex flex-col items-end gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-1 md:pl-10">
                           <div className="text-right">
                              <div className="text-[10px] font-black text-slate-300 line-through tracking-widest uppercase mb-1">Market Avg: ${(basePrice * 1.4 * passengers).toFixed(0)}</div>
                              <div className="flex flex-col items-end">
                                 ...
                                 <div className="text-[10px] font-black text-brand-primary/40 uppercase tracking-widest mt-1">
                                    ${formatCurrency(clientPricePerPerson)} Per Person
                                 </div>
                              </div>
                               <div className="text-[9px] font-black text-slate-600 uppercase mt-4 tracking-widest flex items-center justify-end gap-1 text-right">
                                  ...
                                  Total for {passengers} Traveler{passengers > 1 ? 's' : ''} <br/> (Inc. Tax & Fares)
                               </div>
                           </div>
                           <Link href={`/flights/checkout?flightId=${flight.id}&origin=${depCode}&destination=${arrCode}&priceCents=${Math.round(clientPricePerPerson*passengers*100)}`}
              ...
              {leadStatus !== 'success' ? (
                  ...
                  <form onSubmit={handleLeadSubmit} className="space-y-4">
                     ...
                     <div className="grid grid-cols-2 gap-3">
                        <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex flex-col gap-2 group cursor-pointer hover:bg-slate-100 transition-all"
                          onClick={() => setLeadForm({...leadForm, seats: !leadForm.seats})}>
                           <div className="flex items-center justify-between">
                              <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${leadForm.seats ? 'bg-brand-primary border-brand-primary' : 'border-slate-200'}`}>
                                 {leadForm.seats && <span className="text-white text-[10px] font-black">✓</span>}
                              </div>
                              <div className="text-sm font-black text-brand-primary">+$20</div>
                           </div>
                           <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest block">Seat Lock</span>
                        </div>
                     ...
                     {selectedFlightForLead && (
                       <div className="flex items-center justify-between pt-4 border-t border-slate-100 mt-2">
                          ...
                          <span className="text-xl font-black text-slate-800">
                            ${(calculateClientPrice(parseFloat(selectedFlightForLead.priceTotal), getPriceSeasonality(selectedFlightForLead.arrival?.iataCode || destination, date).multiplier) * passengers).toFixed(2)}
                          </span>
                       </div>
                     )}
                ...
                <div className="text-center py-20 animate-in zoom-in duration-500">
                   ...
                   <p className="text-slate-500 font-medium">
                     ...
                     {leadForm.seats ? ' including your $20 seat reservation' : ''}.
                   </p>
                </div>
              )}
           </div>
      ...
      <div className="bg-brand-sidebar w-full py-20 px-6 text-white text-center border-t border-white/5">
         <h3 className="text-2xl font-black mb-6 italic uppercase tracking-tighter italic">Trusted Global Travel Partner since 2001</h3>
         <p className="text-white/40 text-sm max-w-2xl mx-auto leading-relaxed font-bold">Arranged over 200,000 flights from our Wilmington, Delaware office. ARC and ASTA accredited. 100% brand exclusivity and private contract rates with No-Loss Guarantee.</p>
      </div>
    </div>
  );
}
