  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center py-16 px-4">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div>
              ...
              <div className="font-bold text-slate-800">{booking ? `${booking.firstName} ${booking.lastName}` : (pnr ? 'Confirmed' : '—')}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">PNR / Booking Ref.</div>
              <div className="font-bold text-brand-primary font-mono text-lg tracking-widest">{booking?.bookingReference || pnr || '—'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Airline</div>
              <div className="font-semibold text-slate-800">{booking ? 'Scheduled Carrier' : '—'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Cabin</div>
              <div className="font-semibold text-slate-800">ECONOMY</div>
            </div>
            <div>
      ...
      <p className="text-xs text-slate-400 mt-6 text-center max-w-sm">
        A confirmation has been sent to {booking?.email || 'your email'}. For support, call <a href="tel:+18669617260" className="underline">+1 866 961 7260</a>.
      </p>
    </div>
  );
}
